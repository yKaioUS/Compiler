...
void expandLeftmost(const string& str, const vector<string>& rhs) {
    auto it = find(sentence.begin(), sentence.end(), str);
    if (it == sentence.end()) {
        cout << "leftmost deviation: false!!" << endl;
        exit(1);
    }
    sentence.erase(it);
    sentence.insert(it, rhs.begin(), rhs.end());
    leftmost_output.push_back(joinSentence());
}

void lexer(string prog){	
	int lineNumber = 1, idx = 0;
...
void syntaxError(string s) {
	stringstream ss;
    ss << "Syntax error, line " << currentLine << ", " << s;
...
bool match(type expectedType) {
	if (token_list[currentIdx].t == expectedType) {// 匹配成功，前进到下一个token
		...
		stringstream ss;
		ss << "Syntax error, line " << currentLine << ", missing \"" << expectedStr << "\"";
		errors.push_back(ss.str());
...
string getHeight(int height) {
    return string(height * 3, '-');
}

void program(int height) {
	/* program -> compoundstmt */
	expandLeftmost("program", {"compoundstmt"});
...
void compoundstmt(int height){
	/* compoundstmt -> { stmts } */
	expandLeftmost("compoundstmt", {"{", "stmts", "}"});
...
void stmts(int height) {
	...
	if (tmp.t == TYPE_IF || tmp.t == TYPE_ID || tmp.t == TYPE_WHILE || tmp.t == TYPE_LBRACE) {
		/* stmts -> stmt stmts */
        expandLeftmost("stmts", {"stmt", "stmts"});
		stmt(height + 1);
		stmts(height + 1);
	} else {
		/* stmts -> ε */
        expandLeftmost("stmts", {});
...
void stmt(int height) {
	...
	if (tmp.t == TYPE_IF) {
		/* stmt -> ifstmt */
        expandLeftmost("stmt", {"ifstmt"});
        ifstmt(height + 1);
	} else if (tmp.t == TYPE_WHILE) {
		/* stmt -> whilestmt */
        expandLeftmost("stmt", {"whilestmt"});
		whilestmt(height + 1);
	} else if (tmp.t == TYPE_ID) {
		/* stmt -> assgstmt */
        expandLeftmost("stmt", {"assgstmt"});
		assgstmt(height + 1);
	} else if (tmp.t == TYPE_LBRACE) {
		/* stmt -> compoundstmt */
        expandLeftmost("stmt", {"compoundstmt"});
		compoundstmt(height + 1);
	} else {
...
void ifstmt(int height) {
    ...
    expandLeftmost("ifstmt", {"if", "(", "boolexpr", ")", "then", "stmt", "else", "stmt"});
        ...
        if (match(TYPE_LPAREN)) {
            ...
            if (match(TYPE_RPAREN)) {
                ...
                if (match(TYPE_THEN)) {
                    ...
                    else syntaxError("missing \"else\"");
                } 
                else syntaxError("missing \"then\"");
            } 
            else syntaxError("missing \")\"");
        } 
        else syntaxError("missing \"(\"");
    } 
    else syntaxError("missing \"if\"");
}

void whilestmt(int height) {
	/* whilestmt -> while ( boolexpr ) stmt */
    expandLeftmost("whilestmt", {"while", "(", "boolexpr", ")", "stmt"});
    ...
    else syntaxError("missing \"while\"");
}

void assgstmt(int height) {
	/* assgstmt -> ID = arithexpr ; */
    expandLeftmost("assgstmt", {"ID", "=", "arithexpr", ";"});
    ...
    else syntaxError("missing identifier");
}

void boolexpr(int height) {
	/* boolexpr -> arithexpr boolop arithexpr */
    expandLeftmost("boolexpr", {"arithexpr", "boolop", "arithexpr"});
...
void arithexpr(int height) {
    ...
    expandLeftmost("arithexpr", {"multexpr", "arithexprprime"});
...
void boolop(int height) {
    ...
    if (tmp.t == TYPE_LT || tmp.t == TYPE_GT || tmp.t == TYPE_LE || tmp.t == TYPE_GE || tmp.t == TYPE_EQ) {
        ...
        expandLeftmost("boolop", {op});
        parseTree.push_back(getHeight(height + 1) + op);
        advanceIdx();
    } 
    else syntaxError("missing relational operator");
}

void multexpr(int height) {
	/* multexpr -> simpleexpr multexprprime */
    expandLeftmost("multexpr", {"simpleexpr", "multexprprime"});
...
void simpleexpr(int height) {
    ...
    if (tmp.t == TYPE_ID) {
        ...
        syntaxError("missing simple expression");
        parseTree.push_back(getHeight(height + 1) + "ID");
    }
}
...
void multexprprime(int height) {
    ...
    if (tmp.t == TYPE_MUL || tmp.t == TYPE_DIV) {
        ...
        expandLeftmost("multexprprime", {op, "simpleexpr", "multexprprime"});
        parseTree.push_back(getHeight(height + 1) + op);
        advanceIdx();
        simpleexpr(height + 1);
        multexprprime(height + 1);
...
void arithexprprime(int height) {
    ...
    if (tmp.t == TYPE_PLUS || tmp.t == TYPE_MINUS) {
        ...
        expandLeftmost("arithexprprime", {op, "multexpr", "arithexprprime"});
        parseTree.push_back(getHeight(height + 1) + op);
        advanceIdx();
        multexpr(height + 1);
        arithexprprime(height + 1);
    	...
    	/* arithexprprime -> ε */
        expandLeftmost("arithexprprime", {});
    	parseTree.push_back(getHeight(height + 1) + "E");
	}
}

void output_leftmost_deviation(){
	cout << "leftmost deviation:" << endl;
	for (size_t i = 0; i + 1 < leftmost_output.size(); ++i)
        cout << '\t' << leftmost_output[i] << " =>" << endl;
	
    cout << '\t' << leftmost_output.back() << endl;
}

void ll_parser(string prog) {
	...
	cout << "========== Build Parse Tree ==========" <<endl;
    for (auto& line : parseTree) { // 输出语法树
        cout << line << endl;
    }
    cout<<endl;
    
    cout << "========== LL Parsing ==========" <<endl;
    
    for (auto& msg : errors) { // 输出错误信息
        cout << msg << endl;
    }
    
    output_leftmost_deviation();
    cout<<endl;
}